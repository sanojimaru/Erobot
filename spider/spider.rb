...
ero_sites.each do |domain, site|
  ...
  rss.items.each do |item|
    ...
    page_body = CharlockHolmes::Converter.convert page_body, detection[:encoding], 'UTF-8'

    page = Nokogiri::XML::Document.parse(page_body, nil, detection[:encoding])
    puts page.errors
    break
    ...
    page_image_elements = page.xpath(site[:img_link_xpath])
    puts page_image_elements

    page_images = page_image_elements
